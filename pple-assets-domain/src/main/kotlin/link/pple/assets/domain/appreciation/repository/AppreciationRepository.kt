...
interface AppreciationRepositoryCustom {
    ...
    fun findAll(donorAccountUuid: UUID?, donationUuid: UUID?): List<Appreciation>
}
...
class AppreciationRepositoryImpl : QuerydslRepositorySupport(Appreciation::class.java),
    ...
    override fun findAll(donorAccountUuid: UUID?, donationUuid: UUID?): List<Appreciation> {
        return from(appreciation)
            ...
            .where(
                appreciation.status.eq(Appreciation.Status.ACTIVE)
                    .and(appreciation.donation.uuid.eqFilterNull(donationUuid))
                    .and(donationHistory.donor.uuid.eqFilterNull(donorAccountUuid))
            )
            .fetch()
    }
}
